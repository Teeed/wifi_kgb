...
void add_mac(struct report *report, uint64_t mac, struct tracking_entry *tracking_entry)
{
	...
	client_report->report = xrealloc(client_report->report, client_report->n_report);
...
void report_free(struct report *report)
{
	...
	for(i = 0; i < report->report.n_report; i++)
	{
		...
		{
			free(report_report->probe_tracking[i]);
		}

		for(j = 0; j < report_report->n_ap_ssid_tracking; j++)
		{
			free(report_report->ap_ssid_tracking[i]);
		}
