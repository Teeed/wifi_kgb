...
void radiotap_get_rxpower_channel(struct radiotap_rx_info *radiotap_rx_info, struct ieee80211_radiotap_header *radiotap)
{
	...
	void *radiotap_fields = (void*)radiotap + sizeof(struct ieee80211_radiotap_header);
	void *radiotap_end = (void*)radiotap + radiotap->it_len;


	if(radiotap->it_present & (1<<31)) // bit 31 is set, we will have additional it_present field
	{
	...
	for(i = 0; i < sizeof(radiotap_header_field_lengths)/sizeof(radiotap_header_field_lengths[0]); i++)
	{
		...
		if(radiotap->it_present & (1<<i)) // field present
		{
			switch(i)
			{
